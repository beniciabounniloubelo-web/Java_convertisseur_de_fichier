...
public class TabCaFinale {
    ...
    public static void createTabCaFinale(String code, Queue<TabCaEntree> afile, Map<Integer, String> tabCanonique) {
        ...
        TabCaEntree e1 = afile.poll(); //on a une entree
        ...
        int a = Integer.parseInt(newcode, 2); // on transforme le code qui est un string en int
        a++; //on l'incremente
        String nextcode = Integer.toBinaryString(a); //on le repasse en string

        createTabCaFinale(nextcode, afile, tabCanonique);
    }
}
